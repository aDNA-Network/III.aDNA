...
- `~/lattice/LatticeNetwork.aDNA/who/governance/adr_010_canonical_node_id_schema_three_universes.md` §d 11-row roster + §b canonical-pin policy. Cited from §4.3 (canonical_id resolution).
- `~/lattice/LatticeNetwork.aDNA/who/governance/adr_013_federation_signing_key_infrastructure.md` §a operator-attestation trust model; §b per-purpose-slot pubkey dict + Ed25519 algorithm + SHA-256 fingerprint format; §c source-side keygen tooling (out-of-MD-A2-scope); §e verify-step binding to ADR-004 §a transmission ceremony. Cited from §4.1 / §4.3 / §4.6.
