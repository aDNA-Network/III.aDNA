...
- **State cascade** — charter F3 row → ✅ completed + Phase-2 exit gate checked + `phase: 1 → 2` + `updated: 2026-05-26`; STATE.md frontmatter (`updated` prefix + `last_session`/`prior_session` + tags) + Current Phase lead paragraph (F3-complete prepend + DG-F criteria 5/6 green + Queue F4-next/DP4-gated). No router (`~/aDNA/CLAUDE.md`) or III root CLAUDE.md edit — Campaign State row there already reads "F3 pending"→ left for F4/close-ceremony cascade (Standing Rule 7: router carries routing identity; campaign state lives in STATE.md/charter).
