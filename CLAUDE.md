...
See `STATE.md` for the authoritative operational snapshot. **Six campaigns shipped end-to-end (A + B + C + D + F + G)**; production pin **`v0.5.0` TAGGED** at the Campaign G G6 DG-G close commit 2026-06-23 (declared at G4 2026-05-31, MANIFEST commit `0f06aa6`; annotated tag cut at the close commit per F4/v0.4.0 precedent; predecessor `v0.4.0` tag `7500c19`). **Campaign G ("Operation Atrium") CLOSED 2026-06-23 — DG-G 11/11** — consolidation campaign anchored on net-new ISS (operator-decision-gate) interaction-quality coverage; DP-1..DP-6 cleared; **G1** ISS pack `context_iii_iss_surfaces.md` (6 traps {1,2,3,4,5,7}; a11y Traps 6/8 held; composite 4.00) + **G2** `learning_store` deepening (sd 3→4; composite 4.00) + audit-ingest seam + lattice 1.2.6 + **G3** ISS candidate ledger (`ISS-C6`/`C8`/`C9`; DP-4 candidates-only, canonical INVARIANT) + **G4** III `v0.4.1→v0.5.0` minor bump + ADR-002 §3 sweep (VideoForge/CanvasForge/wga re-pinned @ v0.5.0; ISS-adopter invite fired) + **G5** inspection-grade validation **VERDICT GO** (6/6 recommended L3 residue axes across 3/3 live adopters; Trap 4→3-vault, Trap 7→2-vault; negative control clean) + **G6** DG-G 11/11 + AAR + annotated `v0.5.0` tag; boundary-preserving end-to-end, canonical md5 `5adb0dfa…`/28 invariant, zero new III ADR (count stays 7; ADR-009 REJECTED). Campaign E remains forward-seeded (generalized writing-III; opens when LiteratureForge.aDNA reaches its forge-BUILD phase per MD-X2 cross-vault note §6).
...
7. **Secret hygiene** — `gitleaks` pre-push hook on every push; a **full-history scan is a hard gate before any host move** (ADR-011); a finding blocks the move until purged + the credential is rotated. III's `how/federation/git/.gitleaks.toml` carries a scoped allowlist for 10 confirmed false-positives (`idempotency_key` field-name prose + vendored Obsidian-plugin minified JS); real PATs are never allowlisted.
